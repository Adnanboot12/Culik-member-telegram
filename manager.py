...
def banner():
    ...
    print(f'{lg}Version: {w}2.0{lg} | GitHub: {w}@Adnanboot12{n}')
    print(f'{lg}Telegram: {w}@@Nnuuuyyyy18{lg} | Instagram: {w}@Adnanboot_nt{n}')
...
while True:
    clr()
    banner()
    print(f'{lg}[1] Tambahkan Akun Baru{n}')
    print(f'{lg}[2] Melihat Nomor Yang Terbanned{n}')
    print(f'{lg}[3] Hapus Akun{n}')
    print(f'{lg}[4] Update  Script{n}')
    print(f'{lg}[5] tampilkan Akun Nomor{n}')
    print(f'{lg}[6] Quit{n}')
    a = int(input('\nEnter your choice: '))
    if a == 1:
        new_accs = []
        with open('vars.txt', 'ab') as g:
            number_to_add = int(input(f'\n{lg} [~] Enter Berapa akun yang mau didaftarkan: {r}'))
            for _ in range(number_to_add):
                phone_number = str(input(f'\n{lg} [~] Masukkan nomor: {r}'))
                parsed_number = ''.join(phone_number.split())
                pickle.dump([parsed_number], g)
                new_accs.append(parsed_number)
            print(f'\n{lg} [i] Akun tersimpan di vars.txt')
            clr()
            print(f'\n{lg} [*] Logging in from new accounts\n')
            for number in new_accs:
                c = TelegramClient(f'sessions/{number}', 29104014 , '877e2a912343fceef2b0dda922a727af')
                c.start(number)
        ...
        if not accounts:
            ...
            for account in accounts:
                phone = str(account[0])
                client = TelegramClient(f'sessions/{phone}', 29104014 , '877e2a912343fceef2b0dda922a727af')
                client.connect()
            ...
            if not banned_accs:
                print(f'{lg} Selamat akun ada tidak ada yang terbanned')
            else:
